...
int main(int argc, char *argv[]) {
    ...
    Try
            Env = TEnv(argc, argv, TNotify::StdNotify);
    const TStr InFNm = Env.GetIfArgPrefixStr("-i:", argv[1], "Input graph file (single directed edge per line)");
    ...
    const bool ScaleInitMtx = Env.GetIfArgPrefixBool("-sim:", false,
                                                     "Scale the initiator to match the number of edges");
    const TFlt PermSwapNodeProb = Env.GetIfArgPrefixFlt("-nsp:", 0.6,
                                                        "Probability of using NodeSwap (vs. EdgeSwap) MCMC proposal");
